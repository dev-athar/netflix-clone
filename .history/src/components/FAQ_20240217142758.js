...
export default function FAQ() {
  ...
  const toggleExpand = (segment) => {
    if (expandedSegment === segment) {
      ...
      setIconRotation({ ...iconRotation, [segment]: 90 }); // Set icon rotation angle to 180 degrees when segment is expanded
    }
  };
      ...
      <ul className="max-w-4xl m-auto">
        <li onClick={() => toggleExpand("whatIsNetflix")} className="pb-2">
          <div className="faq-title">
            ...
            <svg
              className={`w-10 transform rotate-${iconRotation["segment"]}`}
              data-slot="icon"
              fill="none"
              stroke-width="1.5"
              stroke="currentColor"
        ...
        <li onClick={() => toggleExpand("whereCanIWatch")} className="pb-2">
          <div className="faq-title">
            ...
            <svg
              className={`w-10 transform rotate-${iconRotation["whatIsNetflix"]}`}
              data-slot="icon"
              fill="none"
              stroke-width="1.5"
              stroke="currentColor"
